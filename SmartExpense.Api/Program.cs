...
// ====================================
// BOOTSTRAP LOGGER
// Catches fatal startup errors before the DI container is ready.
// Replaced by the full Serilog config once builder.Build() runs.
// ====================================
...
// ReadFrom.Services() enables sink/enricher DI injection (e.g. for custom enrichers).
// ====================================
...
// Must be first so every subsequent middleware and controller log line carries the ID
    app.UseMiddleware<CorrelationIdMiddleware>();

// Replaces Microsoft's default request logging with structured Serilog events.
// Fires after the request completes so StatusCode and Elapsed are both known.
    app.UseSerilogRequestLogging(options =>
    {
        options.MessageTemplate =
            "HTTP {RequestMethod} {RequestPath} → {StatusCode} in {Elapsed:0.0000} ms";

        // Attach extra properties that Serilog's built-in enrichment doesn't cover
        ...
        // Health-check polling generates log noise with zero diagnostic value
...
{
    // HostAbortedException is thrown by EF Core design-time tools — not a real crash
